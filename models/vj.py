...
class VjVae(BaseModel):
    ...
    def build(self):
        ...
        self.audios = self.aud_model.inputs
        self.temperature = self.aud_model.temperature
        self.aud_lengths = tf.ones([self.aud_model.batch_size], tf.int32) * self.aud_model.music_length
        # set up synesthetic encoder and decoder
        self.images, self.aud_latents, self.aud_dist = self.build_encoder(self.audios, self.aud_lengths)
        self.reconstructions, self.aud_lengths, self.vis_latents, self.vis_means, self.vis_sigmas = self.build_decoder(self.images)
        ...
        with tf.name_scope('loss'):
            ...
            self.loss = self.aud_model.calc_loss(self.audios, self.reconstructions, self.vis_means, self.vis_sigmas)
        # set up optimizer
    ...
    def run_train_step(self, tf_session, batch):
        temperature = 1.0
        _, loss, recon_loss, latent_loss, summaries = tf_session.run(
                [self.train_op, self.loss, self.aud_model.recon_loss, self.aud_model.latent_loss, self.merge_op],
                feed_dict={
                    self.audios: batch,
    ...
    def run_test_step(self, tf_session, batch, batch_idx, out_path):
        temperature = 0.5
        loss, recon_loss, latent_loss, images, reconstructions = tf_session.run([
            self.loss,
            self.aud_model.recon_loss,
            self.aud_model.latent_loss,
            self.images,
            self.reconstructions], feed_dict={self.images: batch, self.temperature: temperature})
        losses = {'All': loss, 'MSE': recon_loss, 'KL': latent_loss}
        # save original image and reconstruction
        if (self.export_step > 0) and ((batch_idx-1) % self.export_step == 0):
            self.vis_model.save_image(
                    images[0].squeeze(),
                    os.path.join(out_path, str(batch_idx) + '_' + str(self.epoch) + '_original.png')
                    )
            self.aud_model.save_midi(
                    reconstructions[0],
                    os.path.join(out_path, str(batch_idx) + '_' + str(self.epoch) + '_recon.mid')
                    )
            self.aud_model.save_midi(
                    audios[0],
                    os.path.join(out_path, str(batch_idx) + '_' + str(self.epoch) + '_audio.mid')
                    )
        return losses
